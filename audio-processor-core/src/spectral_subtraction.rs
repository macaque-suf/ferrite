...
impl SpectralSubtractor {
    ...
    fn apply_gain_smoothing(&mut self) {
        ...
        // This reduces isolated "sparkle" bins
    ...
    pub fn process_frame(&mut self, input: &[f32], output: &mut [f32]) -> Result<(), ProcessingError> {
        if !self.initialized {
        ...
        let processed = self.fft_processor.process_with_spectrum(input, output, |spectrum| {
            // Extract magnitude only (no phase needed with direct complex multiplication)
            for i in 0..num_bins.min(spectrum.len()) {
        ...
        // The FFT processor may not process all samples immediately due to internal buffering
        // This is expected behavior for streaming processing
    ...
    pub fn process_buffer(&mut self, input: &[f32], output: &mut [f32]) -> Result<(), ProcessingError> {
        ...
        // Process all input at once through the FFT processor
        // The FFT processor handles its own internal buffering and overlap-add
        // It may not process all samples in one go due to internal state
        let mut input_offset = 0;
        let mut output_offset = 0;
        
        while input_offset < input.len() {
            let chunk_size = (input.len() - input_offset).min(8192); // Process in reasonable chunks
            let in_chunk = &input[input_offset..input_offset + chunk_size];
            let out_chunk = &mut output[output_offset..output_offset + chunk_size];
            
            // Process chunk - this updates the internal buffers
            let _ = self.process_frame(in_chunk, out_chunk);
    ...
    /// Range: 0.0 (disabled) - 1.0 (full)
    pub fn set_comfort_noise_gain(&mut self, gain: f32) {
        self.config.comfort_noise_gain = gain.max(0.0).min(1.0);
        if let Some(ref mut gen) = self.comfort_noise_gen {
            gen.set_gain(gain);
        }
    }
